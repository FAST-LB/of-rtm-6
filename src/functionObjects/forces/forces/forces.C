...
Foam::tmp<Foam::volScalarField> Foam::forces::mu() const
{
    ...
    if (obr_.foundObject<icoTurbModel>(icoTurbModel::propertiesName))
    {
        ...
        if (obr_.foundObject<iitpMixture>("mixture"))
        {
            const iitpMixture& mixture =
                obr_.lookupObject<iitpMixture>("mixture");
        ...
        {        
            ...
            const dictionary& FSIDict =
                obr_.lookupObject<IOdictionary>("preciceDict").subOrEmptyDict("FSI");
    ...
    {
        return obr_.lookupObject<volScalarField>("thermo:mu");
    }
    else
    {
...
void Foam::forces::read(const dictionary& dict)
{
    if (active_)
    {
        ...
        patchSet_ =
            mesh.boundaryMesh().findPatchID(dict.lookup("patches"));
...
Foam::vector Foam::forces::momentEff() const
{
    ...
    return fm.second().first()+fm.second().second();
}
